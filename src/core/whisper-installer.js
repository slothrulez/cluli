  ...
  async install({ onProgress } = {}) {
    ...
    if (ffmpeg.found) {
      ...
      const ffmpegMsg = this.platform === 'win32'
        ? 'ffmpeg not found — optional for Cluli (we always pass WAV audio to Whisper). Install later with `winget install ffmpeg` only if you need other formats.'
        : this.platform === 'darwin'
          ? 'ffmpeg not found — optional for Cluli (we always pass WAV audio to Whisper). Install later with `brew install ffmpeg` only if you need other formats.'
          : 'ffmpeg not found — optional for Cluli (we always pass WAV audio to Whisper). Install later with `sudo apt install ffmpeg` only if you need other formats.';
      log(`! ${ffmpegMsg}`);
    }
