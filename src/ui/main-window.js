...
class MainWindowUI {
    ...
    setupEventListeners() {
        ...
        // Make the command bar draggable even when the native frameless-window
        // drag region is unavailable. Interactive controls remain clickable.
    ...
    updateSkillIndicator() {
        ...
        if (skillSpan) {
            ...
            const tooltip = this.isInteractive ? 
                ...
                `${skillName} - Enable interactive mode (Ctrl/Cmd+Shift+I) to navigate`;
            ...
            <div class="bg-gray-900 text-white p-6 rounded-lg max-w-md w-full">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-400 mt-1">
                        Get your API key from: <a href="https://aistudio.google.com/apikey" target="_blank" class="text-blue-400">Google AI Studio</a>
                    </p>
                </div>
    ...
    setupSettingsShortcut() {
        document.addEventListener('keydown', (e) => {
            // Ctrl/Cmd+Shift+S opens settings. Keep Ctrl/Cmd+, as a
            // backwards-compatible settings shortcut.
            const opensSettings = (e.metaKey || e.ctrlKey) &&
                ((e.shiftKey && e.key.toLowerCase() === 's') || e.key === ',');
            if (opensSettings) {
    ...
    setupWindowDrag() {
        const commandTab = document.querySelector('.command-tab');
        if (!commandTab || !window.electronAPI || !window.electronAPI.moveWindow) return;

        const dragThreshold = 4;
        let gesture = null;
        let suppressClick = false;

        commandTab.addEventListener('pointerdown', (event) => {
            if (!this.isInteractive || event.button !== 0) return;

            // Native form controls must retain their normal pointer behavior.
            // Other command items remain clickable unless the pointer crosses
            // the drag threshold, which makes the entire bar practical to move.
            ...
            commandTab.setPointerCapture(event.pointerId);
        });

        commandTab.addEventListener('pointermove', (event) => {
            if (!gesture || event.pointerId !== gesture.pointerId) return;

            const distanceX = event.clientX - gesture.startX;
            const distanceY = event.clientY - gesture.startY;
            if (!gesture.didDrag && Math.hypot(distanceX, distanceY) < dragThreshold) return;

            if (!gesture.didDrag) {
                gesture.didDrag = true;
                commandTab.classList.add('is-dragging');
            }
        ...
        const stopDragging = (event) => {
            if (!gesture || event.pointerId !== gesture.pointerId) return;
            const { pointerId, didDrag } = gesture;
            if (commandTab.hasPointerCapture(pointerId)) {
                commandTab.releasePointerCapture(pointerId);
            }
            commandTab.classList.remove('is-dragging');
            gesture = null;
        ...
        commandTab.addEventListener('lostpointercapture', () => {
            commandTab.classList.remove('is-dragging');
            gesture = null;
        });

        // A drag beginning on a command item must not trigger its click action
        // when the pointer is released; ordinary clicks remain unchanged.
        commandTab.addEventListener('click', (event) => {
    ...
    showSettingsMenu() {
        ...
        const quitOption = this.createMenuItem('Quit Cluli', 'fa-power-off', () => {
